...
- **Mô tả nghiệp vụ:** Thông tin mở rộng của khách hàng, bao gồm tổng chi tiêu, điểm tích luỹ và hạng thành viên hiện tại.

| Attribute (Java) | Column (SQL) | SQL Server Type | JPA Annotation | Ghi chú |
|---|---|---|---|---|
| id | id | BIGINT IDENTITY(1,1) | @Id @GeneratedValue | PK |
| userId | user_id | BIGINT | @Column(nullable=false, unique=true) | FK to User |
| totalSpent | total_spent | DECIMAL(12,0) | @Column(nullable=false) | Sum of paid invoices |
| loyaltyPoints | loyalty_points | INT | @Column(nullable=false) | Points earned (1 point = X VND) |
...
- **Mô tả nghiệp vụ:** Cấu hình các mức hạng thành viên do Admin quản lý, bao gồm ngưỡng chi tiêu, % giảm, tỷ lệ tích điểm.
...
| minSpendThreshold | min_spend_threshold | DECIMAL(12,0) | @Column(nullable=false) | Minimum totalSpent for this rank |
...
| id | id | BIGINT IDENTITY(1,1) | @Id @GeneratedValue | PK |
| userId | user_id | BIGINT | @Column(nullable=false) | FK to User |
| type | type | VARCHAR(20) | @Enumerated(STRING) @Column(nullable=false) | EARN / REDEEM / TOPUP / RANK_UPGRADE |
...
- Rank evaluation: Khi invoice PAID, cộng totalSpent, check threshold → auto-upgrade rank + ghi LoyaltyTransaction
- Rank top-up: Tính số tiền cần nạp = (target_threshold - current_totalSpent), tạo VNPay/MoMo payment, xử lý IPN callback → upgrade rank + ghi RankTopUp + LoyaltyTransaction
- Loyalty points: Quy đổi VND → điểm (theo points_per_thousand_vnd), REDEEM khi dùng điểm thanh toán
...
- Cần Invoice.PAID event từ Dev D để trigger cộng totalSpent + points
...
### Flowchart 4### Flowchart 4 — Tích điểm & Lên hạng

```
┌─────────────────────────────────────────────────────────────────┐
│                    HAI CÁCH LÊN HẠNG                             │
│                                                                 │
│ Hạng: BRONZE(0đ) → SILVER(10M) → GOLD(30M) → PLATINUM(70M)     │
│       → DIAMOND(100M)                                            │
│ Ngưỡng lưu trong bảng customer_rank_config, Admin thay đổi được │
└──────────────────────────┬──────────────────────────────────────┘
                           │
          ┌────────────────┴────────────────┐
          │                                 │
          ▼                                 ▼
┌──────────────────────────────────────────────────────────────────┐
│ CÁCH 1 — Tích luỹ chi tiêu (tự động)                            │
│                                                                  │
│ Trigger: Khi Invoice có paymentStatus = 'PAID'                   │
│   (Dev D gọi rankService.processPaidInvoice(invoiceId))          │
│                                                                  │
│ 1. Đọc Invoice:                                                  │
│    SELECT * FROM invoice WHERE id = ? AND payment_status = 'PAID'│
│                                                                  │
│ 2. Đọc CustomerProfile của user:                                 │
│    SELECT * FROM customer_profile WHERE user_id = invoice.user_id│
│                                                                  │
│ 3. Cộng totalSpent:                                              │
│    newTotal = profile.totalSpent + invoice.totalAmount           │
│    UPDATE customer_profile SET total_spent = newTotal            │
│      WHERE user_id = ?                                           │
│                                                                  │
│ 4. Tính điểm loyalty:                                            │
│    Công thức: pointsEarned = FLOOR(invoice.totalAmount           │
│                  / 1000 * config.pointsPerThousandVnd)           │
│    Ví dụ: totalAmount = 500,000đ, pointsPerThousandVnd = 1       │
│            → pointsEarned = FLOOR(500000 / 1000 * 1) = 500 điểm  │
│    UPDATE customer_profile SET loyalty_points =                  │
│      loyalty_points + pointsEarned WHERE user_id = ?             │
│    INSERT INTO loyalty_transaction (                             │
│      user_id, type='EARN', pointsDelta=+pointsEarned,            │
│      amountReference=invoice.totalAmount,                        │
│      description='Tích điểm từ hoá đơn #' + invoice.id          │
│    )                                                             │
│                                                                  │
│ 5. Kiểm tra lên hạng (loop qua tất cả rank configs):            │
│    SELECT * FROM customer_rank_config WHERE is_active = 1        │
│      ORDER BY min_spend_threshold ASC                            │
│    For each rankConfig:                                          │
│      IF newTotal >= rankConfig.minSpendThreshold                 │
│         AND profile.currentRank.minSpendThreshold                │
│              < rankConfig.minSpendThreshold:                     │
│       → UPDATE customer_profile SET current_rank_id = rankConfig.id│
│         WHERE user_id = ?                                        │
│       → INSERT INTO loyalty_transaction (                        │
│           user_id, type='RANK_UPGRADE',                          │
│           pointsDelta=0,                                         │
│           description='Lên hạng ' + rankConfig.rankName)         │
│       → Gửi email thông báo lên hạng                             │
│       (Chain: nếu newTotal đủ ngưỡng cao hơn nữa → tiếp tục)    │
│                                                                  │
│ 6. Cập nhật discount cho hạng mới (ảnh hưởng lần sau):          │
│    - canBookVip/canBookVvip dùng để gating room selection        │
│    - discountPercent dùng để auto-apply RANK_BENEFIT voucher     │
│                                                                  │
├──────────────────────────────────────────────────────────────────┤
│ CÁCH 2 — Nạp tiền lên hạng ngay (Top-up)                        │
│                                                                  │
│ URL: /customer/rank-topup.jsp                                     │
│                                                                  │
│ 1. Customer thấy danh sách hạng mục tiêu (SILVER→DIAMOND)       │
│    Mỗi hạng hiển thị:                                            │
│      - Tên + lợi ích (discount %, VIP access)                   │
│      - Ngưỡng yêu cầu: XXX,000đ                                 │
│      - Bạn đã chi tiêu: YYY,000đ (totalSpent hiện tại)          │
│      - Số tiền cần nạp: gap = threshold - totalSpent            │
│        (Nếu gap <= 0 → "Bạn đã đủ điều kiện, hãy yêu cầu Admin  │
│         đánh giá lại hạng")                                       │
│                                                                  │
│ 2. Customer chọn hạng → click "Nạp tiền lên hạng"               │
│    Fetch POST /api/rank/topup                                    │
│      Body: {targetRank: 'GOLD', amount: gap}                     │
│                                                                  │
│ 3. Backend tạo payment:                                          │
│    a. INSERT INTO rank_topup (                                   │
│         user_id, target_rank='GOLD', amount=gap,                  │
│         payment_method='VNPAY', status='PENDING'                 │
│       )                                                          │
│    b. Tạo VNPay/MoMo payment request:                            │
│       amount = gap (VND)                                          │
│       txnRef = "TOPUP_" + rankTopup.id + "_" + System.currentTime│
│       returnUrl = "/api/payment/topup-callback"                  │
│    c. Response {paymentUrl: "https://sandbox.vnpay.vn/..."}     │
│    → Redirect customer đến VNPay/MoMo                            │
│                                                                  │
│ 4. IPN callback (BY-PASS AuthFilter):                            │
│    POST /api/payment/topup-callback (hoặc VNPay/MoMo IPN)       │
│    │                                                             │
│    Backend xử lý:                                                │
│    a. Verify chữ ký (hash with secret key)                      │
│    b. Kiểm tra amount khớp với rank_topup.amount                │
│    c. UPDATE rank_topup SET status = 'SUCCESS',                  │
│         transaction_ref = ? WHERE id = ?                         │
│    d. UPDATE customer_profile SET                                │
│         total_spent = total_spent + amount,                      │
│         current_rank_id = targetRankConfig.id                    │
│       WHERE user_id = ?                                          │
│    e. INSERT INTO loyalty_transaction (                          │
│         user_id, type='TOPUP',                                   │
│         pointsDelta=0, amountReference=amount,                   │
│         description='Nạp tiền lên hạng ' + targetRank)           │
│    f. INSERT INTO loyalty_transaction (                          │
│         user_id, type='RANK_UPGRADE',                            │
│         pointsDelta=0,                                           │
│         description='Lên hạng ' + targetRank + ' (top-up)')      │
│    g. Nếu totalSpent mới vượt ngưỡng hạng CAO HƠN nữa:          │
│       → Chain upgrade (lặp lại bước 5 của Cách 1)               │
│    h. Gửi email thông báo: "Chúc mừng bạn đã lên hạng [TÊN]!"   │
│       + lợi ích mới                                              │
│                                                                  │
│ 5. Nếu IPN thất bại:                                             │
│    UPDATE rank_topup SET status = 'FAILED'                       │
│    → Hiển thị trên customer dashboard: "Giao dịch thất bại"     │
└──────────────────────────────────────────────────────────────────┘
```

---

### Flowchart 5### Flowchart 5 — Huỷ đặt bàn & Hoàn cọc
...
│           ├── LoyaltyTransactionType.java   ← EARN, REDEEM, TOPUP, RANK_UPGRADE
...
| **M7: Rank + Loyalty** | W4 | Rank evaluation, auto-upgrade, top-up flow, LoyaltyTransaction logging |
...
CREATE TABLE customer_rank_config (
    ...
    min_spend_threshold     DECIMAL(12,0) NOT NULL,
...
-- 03-seed-data.sql
-- ============================================
INSERT INTO customer_rank_config (rank_name, min_spend_threshold, discount_percent,
    points_per_thousand_vnd, can_book_vip, can_book_vvip, is_active)
VALUES
    ('BRONZE',   0,          0,  1, 0, 0, 1),
    ('SILVER',   10000000,   5,  1, 1, 0, 1),
    ('GOLD',     30000000,   8,  2, 1, 0, 1),
    ('PLATINUM', 70000000,   12, 3, 1, 0, 1),
    ('DIAMOND',  100000000,  15, 5, 1, 1, 1);
...
async function holdTable(tableId) {
    ...
    if (result.success) {
        document.querySelector(`.table-card[data-id="${tableId}"]`).classList.add('selected');
